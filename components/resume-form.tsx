  ...
  const validateStep = (step: number): boolean => {
    ...
    if (step === 0) {
      ...
      formData.workExperience.forEach((exp, idx) => {
        ...
        if (!exp.endDate.trim())
          newErrors[`endDate-${idx}`] = "End date is required";
        if (!exp.description.trim())
          newErrors[`description-${idx}`] = "Description is required";
        exp.highlights.forEach((h, hidx) => {
          if (!h.trim())
            newErrors[`highlight-${idx}-${hidx}`] = "Highlight is required";
        });
      });
    } else if (step === 2) {
      formData.projects.forEach((proj, idx) => {
        if (!proj.name.trim())
          newErrors[`projName-${idx}`] = "Project name is required";
        if (!proj.duration.trim())
          newErrors[`projDuration-${idx}`] = "Duration is required";
        if (!proj.objective.trim())
          newErrors[`projObjective-${idx}`] = "Objective is required";
        proj.highlights.forEach((h, hidx) => {
          if (!h.trim())
            newErrors[`projHighlight-${idx}-${hidx}`] = "Highlight is required";
        });
      });
    } else if (step === 3) {
      formData.leadership.forEach((lead, idx) => {
        if (!lead.position.trim())
          newErrors[`position-${idx}`] = "Position is required";
        if (!lead.organization.trim())
          newErrors[`organization-${idx}`] = "Organization is required";
        if (!lead.duration.trim())
          newErrors[`duration-${idx}`] = "Duration is required";
      ...
      if (!edu.class12School.trim())
        newErrors.class12School = "Class 12 school is required";
      if (!edu.class12Board.trim())
        newErrors.class12Board = "Class 12 board is required";
      if (!edu.class12Marks.trim())
        newErrors.class12Marks = "Class 12 marks are required";
      if (!edu.class10School.trim())
        newErrors.class10School = "Class 10 school is required";
      if (!edu.class10Board.trim())
        newErrors.class10Board = "Class 10 board is required";
      if (!edu.class10Marks.trim())
        newErrors.class10Marks = "Class 10 marks are required";
    } else if (step === 5) {
      formData.coActivities.forEach((act, idx) => {
        if (!act.name.trim())
          newErrors[`actName-${idx}`] = "Activity name is required";
        if (!act.level.trim())
          newErrors[`actLevel-${idx}`] = "Level is required";
        if (!act.achievement.trim())
          newErrors[`actAchievement-${idx}`] = "Achievement is required";
      });
    }
    ...
    <div className="max-w-2xl mx-auto">
      <Card className="p-3 md:p-8">
        <div className="md:mb-8">
          <div className="flex items-center justify-between mb-6 gap-4 md:gap-0 overflow-x-scroll hide-scrollbar">
            {steps.map((step, index) => (
              <div
                key={index}
                className="flex flex-col items-center flex-1 self-baseline"
              >
                <div
                  ...
                  style={
                    index === currentStep ? { backgroundColor: "#ec3a5d" } : {}
                  }
                >
